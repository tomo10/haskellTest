...
-- run parser function is generic so we can always use / reuse it.
-- will take in a string and return some sort of data type (a)
...
tagAttrs t attrs v = 
    concat [otag (unwords $ [t]++(map (\(k,v) -> concat [k,"=\"",v,"\""]) attrs)),v,ctag t]
